## Links

[3D meshes from multiple 2D images](https://en.wikipedia.org/wiki/Comparison_of_photogrammetry_software)

[ImageX Repo](https://github.com/Giantpizzahead/imagex)

[Read The Docs RST Reference](https://sphinx-rtd-theme.readthedocs.io/en/stable/demo/structure.html)

[Example Google Style Python Docstrings](https://www.sphinx-doc.org/en/master/usage/extensions/example_google.html)

[Google Regular Expressions](https://github.com/google/re2/wiki/Syntax)
- Maybe string regular expressions can be applied to image search? (Extend the analogy beyond just the name)

[Image labeling tools](https://www.edge-ai-vision.com/2022/04/exploring-data-labeling-and-the-6-different-types-of-image-annotation/)