...
def save_image(image: np.ndarray, path: Path) -> None:
    ...
    print(f"Image saved to {str(path)}")


def display_image(image: np.ndarray) -> None:
    """Displays an image."""
    print("Displaying image...")
    cv.imshow("Image", cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
    cv.waitKey(0)
...
def draw_rectangle(image: np.ndarray, x: int, y: int, w: int, h: int, color: tuple,
    ...
    """
    cv.rectangle(image, (x, y), (x + w, y + h), color, cv2.FILLED if filled else thickness)
...
def draw_circle(image: np.ndarray, x: int, y: int, r: int, color: tuple,
    ...
    """
    cv.circle(image, (x, y), r, color, cv2.FILLED if filled else thickness)
...
def main():
    ...
    display_image(image)
    # Save the image
    save_image(image, Path("temp/test.png"))
